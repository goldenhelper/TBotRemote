...
class OpenAIService(BaseAIService):
    def __init__(self, api_key: str, model_name: str, video_analyzer=None) -> None:
        """Initializes the OpenAIService class using the Responses API.
        ...
        self.output_tokens_multiplier = 3
    ...
    async def get_response(self, system_prompt: str, *, context_messages: List[ChatMessage] | None = None, query_without_context: str | None = None, flat_history: bool = False) -> Tuple[str, int, int, str | None, Dict[int, str] | None]:
        """
        Makes a request to the OpenAI Responses API, supporting multimodal chat history.
        """
        ...
        try:
            ...
            if context_messages is not None:
                    ...
                    if msg.media_type in ['image', 'sticker'] and msg.media_data:
                        try:
                            ...
                            if len(msg.media_data) > 20 * 1024 * 1024:  # 20MB limit
                                logger.warning(f"Media file too large ({len(msg.media_data)} bytes), using description only")
                                ...
                                logger.info(f"Added image data for message {msg.message_id}")
                        except Exception as e:
                ...
                input_items = query_without_context

            logger.info(f"Sending request to OpenAI Responses API model '{self.model_name}'")
